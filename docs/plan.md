...
M4 之外单独排的一档。第一版做失败了，这里记下失败原因与重做的取舍，
避免下次再走回去。

### 7.5.1 第一版为什么失败

把「一次性历史审计」做成了「实时监控 dashboard」：KPI 大字、顶栏导航、
`<dialog>` 抽屉、动效、tabs、自定义 JS 状态机。
但真实读法是**顺着聚合排行下钻到命令原文，判断值不值得写进 AGENTS.md** ——
这是 explore / drill-down 任务，不是 monitor 任务。
错误的隐喻导致：口径可能被同屏 KPI 并起来看、交互全靠自研 JS、
页面复杂度与它能回答的问题不成比例。
...
| 决策 | 选择 | 理由 |
|---|---|---|
| 结构隐喻 | 分轨道的审计文档 | 口径不可混是硬约束，物理分轨比同屏 KPI 更难出错 |
| 交互 | 原生 `<details>` 下钻 | 语义 HTML 零成本可靠，不用 dialog / tabs / 自定义状态机 |
| 图形 | CSS 横条 + 数值 | 长尾分布下长度对比够用，不引图表库（依赖上限 4 个） |
| 数据量 | 聚合行 + 每行预挂 top-5 样本 | 5.3 万条全量内联约 100 MB，不可行 |
| 模块 | 新建 `src/cmdaudit/viz/`，复用 `report.queries` / `report.scope` | 数字与 `report.md` 同源，不写第二套 SQL |
| CLI | 独立 `viz` 子命令 | 不给 `report` 加 `--html`，报告与页面互不牵连 |

### 7.5.3 结构

- `viz/model.py` — 只描述「页面需要什么」的 dataclass，不含 HTML
- `viz/collect.py` — DuckDB → `Payload`；列名对照 `DESCRIBE commands` 校验 +
  参数绑定；`SAMPLES_PER_ROW=5`、`MAX_CANDIDATES=24`
- `viz/render_html.py` — 单文件离线 HTML；`_esc` 是唯一文本出口；
  仅一段渐进增强 JS（筛选框），JS 失效数据仍完整可读
- `viz/build.py` — 入口 `build_viz()`

### 7.5.4 M5 停止条件

- [x] 三条轨道各自打印口径与 caveat，页面上不存在跨轨道相加的数字
- [x] 耗时轨道样本只含 `self_reported`，失败轨道样本全为 `failed`
- [x] 每个聚合行可下钻到命令原文样本 + 可复现 SQL，样本与聚合行同源
- [x] 候选区标注 `exploratory`，无判决式措辞
- [x] 无 CDN / 外部字体 / 埋点，`file://` 双击可用，未引入新依赖
- [x] 敌意输入（含 `<script>`）经 `_esc` 转义，实测页面内 `<script>` 仅 1 处
- [x] 1440×960 与 390×844 实测无横向溢出，键盘可达

**到这里就停。** 不做健康分、不做趋势动画、不做服务端。
