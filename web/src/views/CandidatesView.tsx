    ...
    <div className="grid gap-3">
      <Card>
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="t-title font-medium tracking-tight">待验证候选</h2>
            ...
            <Card>
              <header className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <code className="mt-2 block break-all font-mono t-body font-medium">
                ...
                <div className="text-right">
                  ...
                  <p className="num t-title" style={{ color: "var(--text-warn)" }}>
              ...
              <div className="mt-3 grid gap-2.5 lg:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 t-body-sm">{candidate.hypothesis}</p>
                </div>
                <div>
                  <p className="t-eyebrow-cjk" style={{ color: "var(--text-faint)" }}>
                    验证设计
                  </p>
                  <p className="mt-1 t-body-sm">{candidate.design || "—"}</p>
                </div>
              </div>

              {Object.keys(candidate.observed).length > 0 ? (
                <dl
                  className="mt-3 grid gap-px overflow-hidden rounded-card sm:grid-cols-3 lg:grid-cols-4"
                ...
                >
                  {Object.entries(candidate.observed).map(([label, value]) => (
                    <div key={label} className="px-2.5 py-2" style={{ background: "var(--bg-inset)" }}>
                      ...
                      <dd className="num mt-0.5 break-all t-mono font-medium">
                        {observedText(value)}
                      </dd>
                    </div>
                  ))}
              ...
              {candidate.caveats.length > 0 ? (
                <ul className="mt-3 grid gap-1">
                  {candidate.caveats.map((caveat) => (
                    <li
                      key={caveat}
                      className="border-l-2 pl-2.5 t-body-sm"
