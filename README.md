...
它不是监控大屏，是一次性历史审计的下钻工具。八个视图按证据轨道分区，
每条轨道自带口径与 caveat；点开任意聚合行可看该行的命令原文样本与可复现 SQL。
数字与 `report.md` 同源同 SQL，两份产物不会给出不同结论。

其中两个视图回答别处看不到的问题：

- **重试循环** 是唯一按会话聚合的视图。别处按命令模板或 agent 汇总，
  于是「同一次会话里把同一条命令重来了 165 遍」这个事实在别处不可见。
  注意重试本身不等于错误 —— 轮询等待、逐段读文件都是正常工作方式，
  页面对此有显式口径声明，真正值得看的是失败占比高的链。
- **命令构成** 按动作类别看失败率，纠正按绝对失败数排序的偏差：
  `build` 类失败率是 `search_read` 的十倍，但后者体量大 60 倍，
  在按模板排的队列里会把前者彻底淹没。
